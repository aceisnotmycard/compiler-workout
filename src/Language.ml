  ...
  struct
    ...
    *)                                                       
      ...
      | Const c -> 
        (st, i, o, Some c)
      | Var var -> 
        (st, i, o, Some (State.eval st var))
      | Binop (op, l, r) -> 
        let ((st', i', o', Some l') as conf) = eval env conf l in
        let (st'', i'', o'', Some r') = eval env conf r in
  ...
  struct
    ...
    *)
    let rec eval env ((st, i, o, r) as conf) k stmt = 
      ...
      | Read var -> 
        let (z::i') = i in
        eval env (State.update var z st, i', o, r) Skip k
      | Write e -> 
        let (st', i', o', Some rv) = Expr.eval env conf e in
        eval env (st', i', o' @ [rv], r) Skip k
      | Assign (var, e) ->
        let (st', i', o', Some rv) = Expr.eval env conf e in
        eval env (State.update var rv st', i', o', r) Skip k
      ...
      | If (e, tru, fls) ->
        let (st', i', o', Some rv) = Expr.eval env conf e in
        eval env (st', i', o', r) k (if (rv = 0) then fls else tru) 
      | While (e, s) ->
        let (st', i', o', Some rv) = Expr.eval env conf e in
        (if (rv = 0)
      ...
      | Call (fn, args) -> eval env (Expr.eval env conf (Expr.Call (fn, args))) k Skip
